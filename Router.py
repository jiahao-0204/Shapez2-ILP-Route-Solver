...
class Router:
    ...
    def initialize_board(self, width, height, jump_distances, num_nets):
        ...
        for edge in self.all_edges:
            ...
            self.model.addGenConstrIndicator(self.is_edge_used[edge], True, self.edge_flow_value[edge] >= 1)
            
            # if edge is not used by any net, flow value must be 0
            self.model.addGenConstrIndicator(self.is_edge_used[edge], False, self.edge_flow_value[edge] == 0)
        ...
        for node in self.all_nodes:
            ...
            self.is_node_used_by_belt[node] = self.model.addVar(name=f"node_{node}", vtype=GRB.BINARY)
