...
class Router:
    ...
    def initialize_board(self, width, height, jump_distances, num_nets):
        ...
        for node in self.all_nodes:
            ...
            node_belt_edges_bool_list = [self.is_edge_used[edge] for edge in self.node_related_belt_edges[node]]
            self.is_node_used_by_belt[node] = self.model.addVar(name=f"used_by_belt_{node}", vtype=GRB.BINARY)
            self.model.addGenConstrOr(self.is_node_used_by_belt[node], node_belt_edges_bool_list)
    ...
    def add_belt_pad_net_overlap_constraints(self):
        ...
        for node in self.all_nodes:
            ...
            list_of_things_using_node += [self.is_node_used_by_belt[node]]
            list_of_things_using_node += [self.is_edge_used[edge] for edge in self.node_related_starting_pad_edges[node]]
            list_of_things_using_node += [self.is_edge_used[edge] for edge in self.node_related_landing_pad_edges[node]]
        ...
        for node in self.all_nodes:
            for i in range(self.num_nets):
                self.is_node_used_by_net[i][node] = self.model.addVar(name=f"used_by_net_{i}_{node}", vtype=GRB.BINARY)
                self.model.addGenConstrIndicator(self.is_node_used_by_net[i][node], True, self.node_in_flow_value_expr[i][node] + self.node_out_flow_value_expr[i][node] >= 1)
                self.model.addGenConstrIndicator(self.is_node_used_by_net[i][node], False, self.node_in_flow_value_expr[i][node] + self.node_out_flow_value_expr[i][node] == 0)
            
            # at most one net can use a node
            self.model.addConstr(quicksum(self.is_node_used_by_net[i][node] for i in range(self.num_nets)) <= 1)
    ...
    def add_source_node_constraints(self, node:Node, direction:Direction):
        ...
        for edge in self.node_in_flow_edges[node]:
            if edge[2] == (-direction[0], -direction[1]):
                self.model.addConstr(self.is_edge_used[edge] == 0)
    ...
    def solve(self, timelimit, option, live_draw = False):
        ...
        if self.model.SolCount > 0:
            used_edge = {edge for edge in self.all_edges if self.is_edge_used[edge].X > 0.5}
            self.draw(used_edge)
    ...
    def draw_solution_callback(self, model, where):
        ...
        if where == GRB.Callback.MIPSOL:
            used_edge = [edge for edge in self.all_edges if model.cbGetSolution(self.is_edge_used[edge]) > 0.5]
    ...
    def draw_edges(self, ax, used_edge):
        ...
        used_step_edges = [edge for edge in self.step_edges if edge in used_edge]
        used_jump_edges = [edge for edge in self.jump_edges if edge in used_edge]
